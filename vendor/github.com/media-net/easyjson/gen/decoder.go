...
import (
	...
	"github.com/media-net/easyjson"
	"github.com/media-net/cargo/pool"
)
...
func (g *Generator) genTypeDecoderNoCheck(t reflect.Type, out string, tags fieldTags, indent int) error {
	...
	case reflect.Slice:
		...
		if elem.Kind() == reflect.Uint8 {
			...
			fmt.Fprintln(g.out, "// just test comment")
			fmt.Fprintln(g.out, ws+"if in.IsNull() {")
			fmt.Fprintln(g.out, ws+"  in.Skip()")
			fmt.Fprintln(g.out, ws+"  "+out+" = nil")
			fmt.Fprintln(g.out, ws+"} else {")
			...
			if elem.Implements(reflect.TypeOf((*pool.ReferenceCountable)(nil)).Elem()) {
				fmt.Fprintln(g.out, ws+"    "+tmpVar+" := Acquire"+strings.Replace(g.getType(elem), "*", "", -1)+"()")
			} else {
	...
	case reflect.Ptr:
		fmt.Fprintln(g.out, ws+"if in.IsNull() {")
		fmt.Fprintln(g.out, ws+"  in.Skip()")
		fmt.Fprintln(g.out, ws+"//"+reflect.TypeOf((*pool.ReferenceCountable)(nil)).Elem().String()+" "+t.Elem().String())
		if t.Implements(reflect.TypeOf((*pool.ReferenceCountable)(nil)).Elem()) {
			fmt.Fprintln(g.out, ws+"if "+out+" != nil {")
			fmt.Fprintln(g.out, ws+ws+"  "+out+".DecrementReferenceCount()")
			fmt.Fprintln(g.out, ws+ws+"  "+out+" = nil")
			fmt.Fprintln(g.out, ws+"}")
		} else {
			fmt.Fprintln(g.out, ws+"  "+out+" = nil")
		}
		...
		fmt.Fprintln(g.out, ws+"  if "+out+" == nil {")
		if t.Implements(reflect.TypeOf((*pool.ReferenceCountable)(nil)).Elem()) {
			fmt.Fprintln(g.out, ws+"    "+out+" = Acquire"+strings.Replace(g.getType(t.Elem()), "*", "", -1)+"()")
		} else {
...
func (g *Generator) genStructDecoder(t reflect.Type) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		fmt.Fprintln(g.out, "if out."+f.Name+"!=nil {")
